...
public class MappedFlowAndIpEnrichmentTopology {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // analysis bolt 的 id  (8)
            String[] analysisIDs = new String[topicMapEnrichInputArray.length];
            ...
            for(int i = 0; i < topicMapEnrichInputArray.length; i++) {
                ...
                analysisIDs[i] = ANALYSIS_BOLT_ID + topicInput;
            }
            ...
            for(int j = 0; j < topicMapEnrichOutputArray.length; j++) {
                ...
                logger.info("-----------------------the out topic name is-----{}---------------------", topicOutput);

                String maprichId = MAPPING_ENRICHMENT_BOLT_ID + topicOutput;

                // 每个富化 bolt 连接 8 个  analysis bolt
                InputDeclarer inputDeclarer = topologyBuilder.setBolt(maprichId, mapEnrichBolts.get(topicOutput), MAPPING_ENRICHMENT_BOLT_THREADS)
                        .localOrShuffleGrouping(analysisIDs[1], analysisIDs[1] + maprichId);
                if(analysisIDs.length > 1){
                    for(int i = 1; i < analysisIDs.length; i++){
                        inputDeclarer = inputDeclarer.localOrShuffleGrouping(analysisIDs[i], analysisIDs[i] + maprichId);
                    }
                }


//                logger.info("***************the analysis is"+ analysisIDs + ";the mapenrich is: " + maprichId +
//                        ";the streaming id is： " + analysisIDs.toString()+maprichId + "***********************");


                logger.info("--------------contect the producer boltid is " + KAFKA_PRODUCER_BOLT_ID + topicOutput);
                topologyBuilder.setBolt(KAFKA_PRODUCER_BOLT_ID + topicOutput, kafkaProducerBolts.get(topicOutput),
                        KAFKA_BOLT_THREADS).localOrShuffleGrouping(MAPPING_ENRICHMENT_BOLT_ID + topicOutput);
