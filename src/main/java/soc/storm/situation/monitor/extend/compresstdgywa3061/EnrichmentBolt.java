...
import soc.storm.situation.protocolbuffer.AddressBookProtosTDGYWA3061.SENSOR_LOG;
...
public class EnrichmentBolt extends BaseRichBolt {
    ...
    //
    // private static DateFormat fmt_timestamp = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSSZ");

    static {
        // fmt_timestamp.setTimeZone(TimeZone.getTimeZone("GMT+8"));
            ...
            for (Object skyeyeWebFlowLogByteArrayElement : pbBytesWebFlowLogList) {
                ...
                if (StringUtils.isNotBlank(skyeyeWebFlowLogStr)) {
                    ...
                    if (null != skyeyeWebFlowLog) {
                        ...
                        // skyeyeWebFlowLog.put("es_timestamp", fmt_timestamp.format(new Date()));
                        skyeyeWebFlowLog.put("es_timestamp", Long.valueOf(System.currentTimeMillis()));
                        // skyeyeWebFlowLog.put("@timestamp", Long.valueOf(System.currentTimeMillis()));

                        // 添加es_version字段 “1” add zhongsanmu 20180123
                        skyeyeWebFlowLog.put("es_version", "1");
                        // skyeyeWebFlowLog.put("@version", "1");
